...
# Kelimeleri ve tanımları karıştır
kelimeler = list(sozluk.keys())
tanımlar = list(sozluk.values())
random.shuffle(kelimeler)
random.shuffle(tanımlar)

# Kullanıcı seçimlerini izlemek için session_state kullan
...
st.title("Eşleştirme Oyunu")
st.write("Kelimeleri doğru anlamlarıyla eşleştirin.")

# Seçim yapmak için bir form
with st.form("matching_form"):
    selections = {}
    
    # Kelimeler ve tanımları dropdown ile eşleştirme
    for word in kelimeler:
        selections[word] = st.selectbox(f"{word} kelimesi için anlam seçin:", ["Seç"] + tanımlar)
    ...
    if submitted:
        ...
        for word, selected_definition in selections.items():
            if sozluk[word] == selected_definition:
                correct_matches += 1

        # Puanlama ve geri bildirim
        st.session_state.score += correct_matches
        st.write(f"Doğru eşleştirmeler: {correct_matches}")
        st.write(f"Toplam Puan: {st.session_state.score}")
        st.write(f"Toplam Deneme: {st.session_state.attempts}")

        # Eğer tüm kelimeler doğru eşleştiyse oyun bitir
        if correct_matches == len(sozluk):
            st.success("Tebrikler! Tüm kelimeleri doğru eşleştirdiniz!")
        else:
            st.info("Bazı kelimeler yanlış eşleşti, tekrar deneyin!")
